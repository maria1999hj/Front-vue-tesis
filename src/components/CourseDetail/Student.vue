  ...
  <div>
    ...
    <q-dialog v-model="openModal">
      <q-card style="width: 350px">
        ...
        >
          <q-item-label header>Alumnos no registrados</q-item-label>
...
import { CODE_STUDENT } from "src/constants";
  ...
  setup() {
    ...
    const getStudents = async () => {
      ...
      const result = await api.get(
        `/course/${route.params.course}/user?type_user=${CODE_STUDENT}`
      );
    ...
    const getStudentsNotRegister = async () => {
      ...
      const result = await api.get(
        `/course/${route.params.course}/user?not_user_register=true&type_user=${CODE_STUDENT}`
      );
    ...
    const deleteStudentCourse = async (idStudent) => {
      ...
      await getStudents();
      NotifySuccess("Alumno eliminado del curso");
    };
